...
extension PassiveAuthentication {
    /// A simple test function that will read in a file - test_sod.der containing a SOD object in DER format and try to verify it
    /// Useful for checking things locally
    func test() {
        do {
            let url = Bundle.main.url(forResource: "test_sod", withExtension: "der")!
            let pkcs7Der = try! Data(contentsOf: url)
            let cert = try! getX509CertificateFromPKCS7(pkcs7Der: pkcs7Der )
            ...
            let data = try getSignedDataFromPKCS7(pkcs7Der: pkcs7Der)
