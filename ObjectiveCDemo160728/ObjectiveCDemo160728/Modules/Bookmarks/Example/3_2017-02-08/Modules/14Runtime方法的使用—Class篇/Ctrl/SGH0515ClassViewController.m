...
#import "SGH0515ClassPerson.h"

#define SelfClass [self class]

@interface SGH0515ClassViewController ()
@property(nonatomic,strong)SGH0515ClassPerson *person;
@end

@implementation SGH0515ClassViewController
...
-(void)initial:(NSString *)str {
    ...
    _person = [[SGH0515ClassPerson alloc]init];
    _person.name = @"sengolnhuang";
    _person.age = @"24";
    _person.gender = @"male";
...
- (void)object_dispose:(id)objc {
    SGH0515ClassPerson *tempPerson = [[SGH0515ClassPerson alloc] init];
...
- (const char *)property_copyAttributeValue:(objc_property_t)property attributeName:(const char *)attributeName {
    char * attributeValue = property_copyAttributeValue(property,attributeName);
