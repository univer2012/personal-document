...
`sys.path` 输出是一个列表，其中第一项是空串''，代表当前目录（若是从一个脚本中打印出来的话，可以更清楚地看出是哪个目录），亦即我们执行python解释器的目录（对于脚本的话就是运行的脚本所在的目录）。 
 ...
 <u>了解了搜索路径的概念，就可以在脚本中修改`sys.path`来引入一些不在搜索路径中的模块。 </u>



> 注意：
>
> 比如你要在解释器中执行的`import fibo`，该文件在`/Users/yuanping/Desktop/HuangSengoln/personal_document/workspace/PycharmProjects/Demo1/venv`目录，但是这个目录不在`sys.path`列举的目录中，那么需要执行如下操作：
>
> ```python
> >>> import sys
> >>> sys.path.append("/Users/yuanping/Desktop/HuangSengoln/personal_document/workspace/PycharmProjects/Demo1/venv")
> >>> print(sys.path)
> ```
>
> 再查看该目录是否在其中，如果在，那么就可以`import fibo`了。
>
> 参考：[Python解释器中执行自定义路径的.py文件（模块）win7系统](https://www.cnblogs.com/360FLIP/p/3910842.html)
...
```
(venv) yuanpingdeMacBook-Air:venv yuanping$ python using_name.py 
程序自身在运行
(venv) yuanpingdeMacBook-Air:venv yuanping$ 
```
...
['__annotations__', '__builtins__', '__doc__', '__loader__', '__name__', '__package__', '__spec__', 'a', 'f', 'fib', 'fib2', 'fibo', 'hello', 'i', 'it', 'languages', 'list', 'print_func', 'sys', 'using_name', 'x']
>>> a = 5				# 建立一个新的变量 'a'
>>> dir()
['__annotations__', '__builtins__', '__doc__', '__loader__', '__name__', '__package__', '__spec__', 'a', 'f', 'fib', 'fib2', 'fibo', 'hello', 'i', 'it', 'languages', 'list', 'print_func', 'sys', 'using_name', 'x']
>>> del a			# 删除变量名a
>>> dir()
['__annotations__', '__builtins__', '__doc__', '__loader__', '__name__', '__package__', '__spec__', 'f', 'fib', 'fib2', 'fibo', 'hello', 'i', 'it', 'languages', 'list', 'print_func', 'sys', 'using_name', 'x']
>>> 
```
