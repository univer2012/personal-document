...
- (void)viewDidLoad {
   ...
   // [avAudioPlayer prepareToPlay];
     ...
     */
    UIVisualEffectView *blackView=[[UIVisualEffectView alloc]initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleDark]];
    blackView.backgroundColor=[UIColor blackColor];
    blackView.frame=CGRectMake(0, 0, WIDTH, HEIGHT);
    blackView.alpha=0.7;
    [self.view addSubview:blackView];
    ...
    [scrollViewLeft setBackgroundColor:[UIColor clearColor]];
...
//        isFirst=NO;
        ...
        [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionDefault];//kCAMediaTimingFunctionEaseInEaseOut];
    ...
    CGFloat endAngle=[(NSNumber *)[cdView valueForKeyPath:@"layer.transform.rotation.x"] floatValue];
    ...
    if (isPlay) {
        ...
        [cdView.layer removeAllAnimations];
    }
    isPlay=!isPlay;
}
//播放 进度
...
{
    CGSize size =[_string boundingRectWithSize:CGSizeMake(MAXFLOAT, _lable.frame.size.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:nil context:nil].size;
    //[_string sizeWithFont:_lable.font constrainedToSize:CGSizeMake(MAXFLOAT, _lable.frame.size.height)];
    return size;
}
// scrollView 已经滑动
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if(scrollView.contentOffset.x==WIDTH) {
        ...
        if (scrollView.contentOffset.x>=1.5*WIDTH) {
            pageControl.currentPage=2;
        }else {
            pageControl.currentPage=1;
        }
        middleView.alpha=1-(scrollView.contentOffset.x-WIDTH)/WIDTH;//WIDTH/scrollView.contentOffset.x-0.5;
    }else if(scrollView.contentOffset.x<WIDTH) {
        ...
        middleView.alpha=1-(WIDTH-scrollView.contentOffset.x)/WIDTH;
    }
}
